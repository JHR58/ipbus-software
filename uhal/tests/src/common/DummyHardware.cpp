    ...
    mBigEndianHack ( aBigEndianHack )
{
  ...
  std::string line;
  std::ifstream myfile ("register-values.txt");
  if (myfile.is_open())
  {
    std::stringstream ss;
    while ( getline (myfile,line) )
    {
      ...
      unsigned int addrNum = std::stoul(addr, nullptr, 16);
      unsigned int valNum = std::stoul(val, nullptr, 16);
      //unsigned int x = std::stoul(val, nullptr, 16);



      log ( Debug() , static_cast<uint32_t>(addrNum));
      log ( Debug() , valNum);
      SetEndpoint(addrNum, valNum);
    }
    myfile.close();
  }
  //SetEndpoint( 0x20000005 ,10);
...
{
  log ( Debug() , "\ndata" );
  log ( Debug() , aAddress & ADDRESSMASK );
  log ( Debug() , "\ndata" );
  log ( Debug() , aValue);
  if( ! mMemory.size() )
